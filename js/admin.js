...
// Versi: 2.1 - Card Render Fix + Manifest Fix + System Health
// ============================================================
...
(function initManifest() {
    try {
        const manifest = {
            ...
            start_url: "./admin.html",        // ✅ FIX: Valid relative URL (bukan ".")
            scope: "./",                       // ✅ Valid scope
            display: "standalone",
...
async function safeFetchJSON(url, opts = {}, timeout = APP_CONFIG.FETCH_TIMEOUT, retries = 2) {
    for (let i = 0; i <= retries; i++) {
        try {
            ...
            // ✅ Deteksi 404 spesifik (deployment GAS tidak aktif)
            if (res.status === 404) {
            ...
            // ✅ JANGAN retry untuk 404 (bukan masalah jaringan)
...
// ============ ✅ LOAD DASHBOARD (Tanpa session cache yang stale) ============
...
// ============ ✅ TAB SWITCHING (FIXED: Render tab yang dibuka) ============
function switchTab(tab) {
    ...
    // ✅ FIX: Render ulang tab personel agar sesuai currentView (list/card)
...
// ============ ✅ VIEW TOGGLE (FIXED: Re-render setelah ganti mode) ============
function setView(view) {
    ...
    // ✅ FIX KRITIS: Render ULANG agar grid/table terisi sesuai mode aktif
    renderPegawai();
    renderKorlap();
}

// ============ RENDER PERSONEL (Smart render, hanya view aktif) ============
