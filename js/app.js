...
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbw3rVOUuD6viUevnvcI-JcCg114syXT1IZRy6QegkTR1z05vqnKm7H4TGAgbxJ1_O8iag/exec";
...
// ============ ✅ PATCH: BANNER STATUS ABSEN ============
function setBanner(icon, title, desc, href, ctaText, cls) {
    const b = document.getElementById('absenBanner');
    if (!b) return;
    document.getElementById('absenBannerIcon').innerHTML = `<i data-lucide="${icon}" size="26"></i>`;
    document.getElementById('absenBannerTitle').innerText = title;
    document.getElementById('absenBannerDesc').innerText = desc;
    const cta = document.getElementById('absenBannerCta');
    if (ctaText && href && href !== '#') {
    ...
    b.className = 'absen-banner' + (cls ? ' ' + cls : '');
    b.style.display = 'flex';
    lucide.createIcons();
}

// ============ ✅ PATCH: INIT BANNER (Fetch Status dari Server) ============
async function initAbsenBanner() {
    const b = document.getElementById('absenBanner');
    if (!b) return;
    
    const last = JSON.parse(localStorage.getItem('pusda_last_pegawai') || 'null');
    
    // Jika belum pernah absen sama sekali
    if (!last) {
        setBanner('fingerprint', 'Mulai Presensi Digital', 'Ketuk untuk melakukan absen kehadiran', 'presensi.html', 'MULAI');
        return;
    }
    
    // Loading state
    setBanner('loader', 'Halo, ' + last.nama, 'Mengambil status absen...', '#', '');
    
    try {
        const d = await safeFetchJSON(SCRIPT_URL + '?action=getTodayPresensi', {}, 10000);
        const logs = (d.data || []).filter(l => String(l.id_pegawai) === String(last.id));
        
        let hadir = null, pulang = null, special = null;
        logs.forEach(l => {
            const s = (l.status || '').toLowerCase();
            const j = new Date(l.timestamp).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' });
            if (s.includes('pulang')) pulang = pulang || j;
            else if (s.includes('hadir') || s.includes('terlambat')) hadir = hadir || j;
            else if (['izin', 'sakit', 'dinas'].includes(s)) special = s;
        });
        
        if (special) {
            setBanner('file-text', 'Status: ' + special.toUpperCase(), 'Tidak perlu absen kehadiran hari ini', '#', '', 'ok');
        } else if (pulang) {
            setBanner('check-circle', 'Presensi Lengkap ✔', 'Hadir ' + hadir + ' • Pulang ' + pulang, '#', '', 'ok');
        } else if (hadir) {
            setBanner('sun', 'Sudah HADIR ' + hadir, 'Jangan lupa absen PULANG sebelum selesai', 'presensi.html', 'PULANG', 'ok');
        } else {
            setBanner('alert-circle', 'Belum Absen Hari Ini', 'Halo ' + last.nama + ', ketuk untuk absen sekarang', 'presensi.html', 'ABSEN');
        }
    } catch (e) {
        setBanner('wifi-off', 'Status tidak tersedia', 'Periksa koneksi / buka presensi manual', 'presensi.html', 'BUKA');
    }
}
...
window.onload = () => {
    ...
    // ✅ PATCH: ONLINE/OFFLINE DETECTION
    window.addEventListener('offline', () => {
        showToastError('Offline', 'Koneksi terputus. Data cache tetap tersedia.');
    });
    window.addEventListener('online', () => {
        silentBackgroundUpdate();
        initAbsenBanner(); // Update banner saat koneksi pulih
    });
    ...
    // ✅ PATCH: Initialize banner status absen
    initAbsenBanner();
};
