// ============================================================
// PRESENSI.JS - FRONTEND LENGKAP (v5.4.2)
// Halaman: presensi.html
// ============================================================
...
// ============================================================
// PWA MANIFEST (Blob URL - Anti Warning Chrome)
// ============================================================
try {
    const manifest = {
    ...
    console.log('✅ Presensi Manifest OK');
...
// VARIABEL GLOBAL
// ============================================================
let isFaceApiLoaded = false, isFaceApiLoading = false, isInitialMapBound = false, _lastFrameTime = 0;
let isFormLoading = false;
let _lastToastKey = '', _lastToastTime = 0;

// ============================================================
// ✅ MONITORING & PERSONALISASI
// ============================================================
function logPerformance(action, ms) {
    try {
        const log = JSON.parse(localStorage.getItem('pusda_perf_log') || '[]');
        log.push({ action, ms: Math.round(ms), time: Date.now() });
        localStorage.setItem('pusda_perf_log', JSON.stringify(log.slice(-50)));
    } catch (e) { }
}

function saveLastPegawai(p) {
    try {
        localStorage.setItem('pusda_last_pegawai', JSON.stringify({
            id: p.ID || p.id,
            nama: p.Nama || p.nama
        }));
    } catch (e) { }
...
const STATUS_CONFIG = {
    ...
    'IZIN': { placeholder: 'Jelaskan alasan izin (cuti, urusan keluarga, dll)...', title: '📝 IZIN', message: 'Silahkan hubungi Koordinator / Pimpinan untuk meminta izin.', icon: 'file-text', color: '#d8b4fe', borderColor: '#a855f7', actions: [{ label: 'Lampirkan Surat', icon: 'paperclip', action: 'uploadSurat' }] },
    'SAKIT': { placeholder: 'Jelaskan kondisi sakit dan lampirkan surat dokter...', title: '🏥 SAKIT', message: 'Lampirkan foto surat keterangan dokter.', icon: 'heart-pulse', color: '#fde047', borderColor: 'var(--warning)', actions: [{ label: 'Lampirkan Surat Dokter', icon: 'paperclip', action: 'uploadSurat' }] },
    'DINAS': { placeholder: 'Jelaskan lokasi dan tujuan dinas...', title: '💼 DINAS', message: 'Lampirkan foto surat tugas dinas.', icon: 'briefcase', color: '#fdba74', borderColor: 'var(--accent)', actions: [{ label: 'Lampirkan Surat Tugas', icon: 'paperclip', action: 'uploadSurat' }] },
    'QUICK RESPONSE': { placeholder: 'Tuliskan ringkasan tugas darurat dengan detail...', title: '⚡ QUICK RESPONSE', message: 'Tuliskan ringkasan tugas hari ini dengan detail.', icon: 'zap', color: '#f9a8d4', borderColor: '#ec4899', actions: [] }
};
...
window.onload = () => {
    ...
    window.addEventListener('offline', () => {
        showToast("Offline", "Koneksi terputus. Data cache tetap tersedia.", "warning");
    });
    window.addEventListener('online', () => {
        showToast("Online", "Koneksi pulih. Sinkronisasi ulang...", "success");
...
function showUpdateModal() {
    ...
    document.getElementById('notifMessage').innerText = "Versi terbaru aplikasi E-PUSDA telah dirilis.";
...
function fetchWithTimeout(url, options = {}, timeout = 15000) {
    ...
    const id = setTimeout(() => controller.abort(new DOMException('Timeout ' + timeout + 'ms', 'AbortError')), timeout);
...
function upLoc() {
    ...
    navigator.geolocation.getCurrentPosition(p => {
        if (p.coords.accuracy > 250) {
            sndError.play(); showToastOnce('gps_lemah', "Sinyal Terlalu Lemah", `Akurasi GPS ${p.coords.accuracy.toFixed(0)}m (>250m). Presensi ditolak.`, "error");
            ...
            uPos = { lat: 0, lng: 0 }; updateWorkflow(); return;
        }
        if (p.coords.accuracy > 150) { showToastOnce('gps_warning', "Peringatan Sinyal", `Akurasi GPS ${p.coords.accuracy.toFixed(0)}m.`, "warning"); }
...
function showPermissionModal(type) {
    ...
    if (type === 'camera') {
        ...
        s.innerHTML = '<div class="permission-step"><div class="permission-step-num">1</div><div>Klik <b>Coba Lagi</b></div></div><div class="permission-step"><div class="permission-step-num">2</div><div>Pilih <b>Izinkan</b></div></div>';
...
function loadFromCache() {
    ...
    if (c) {
        try { dbE = JSON.parse(c); dbF = [...dbE]; renderChips(); upUI(); return true; }
...
async function loadData() {
    ...
    try {
        ...
        showToastOnce('load_fail', "Koneksi Gagal", "Tidak dapat terhubung ke server.", "error");
    } finally {
        const o = document.getElementById('initialLoadingOverlay');
        if (o) { o.style.opacity = '0'; o.style.pointerEvents = 'none'; setTimeout(() => o.style.display = 'none', 400); }
    }
...
// ============================================================
// ✅ CEK STATUS ABSEN (FIXED: Kenali semua variasi status)
// ============================================================
function checkAtt(id, st) {
    return dbP.some(l => {
        const lid = String(l['ID Pegawai'] || l.id_pegawai || l.ID || '');
        const rawStatus = l.Status || l.status || '';
        const ls = String(rawStatus).toLowerCase().trim();
        ...
        if (st === 'QR_HADIR') {
            return ls === 'qr hadir' || (ls.includes('qr') && ls.includes('hadir'));
        }
        if (st === 'QR_PULANG') {
            return ls === 'qr pulang' || (ls.includes('qr') && ls.includes('pulang'));
        }
        if (st === 'SPECIAL') {
            return ls === 'izin' || ls === 'sakit' || ls === 'dinas';
        }
        return false;
    });
}
...
// ============================================================
// ✅ SET STATUS PRESENSI (Dengan validasi lengkap)
// ============================================================
function setS(el, st) {
    ...
    // ✅ CEK STATUS KHUSUS (IZIN/SAKIT/DINAS) - Block semua presensi jika sudah ada
    const hasSpecial = checkAtt(pid, 'SPECIAL');

    if (hasSpecial && !['IZIN', 'SAKIT', 'DINAS'].includes(st)) {
        sndError.play();
        showToast("Tidak Dapat Presensi", "Anda sudah mengajukan IZIN/SAKIT/DINAS hari ini. Tidak dapat melakukan presensi lain.", "error");
        return;
    }

    // ✅ CEK STATUS KHUSUS - Tidak boleh diajukan 2x
    if (['IZIN', 'SAKIT', 'DINAS'].includes(st)) {
        if (hasSpecial) {
            sndError.play();
            showToast("Sudah Diajukan", "Anda sudah mengajukan status khusus hari ini.", "error");
            return;
        }

        if (checkAtt(pid, 'HADIR')) {
            sndError.play();
            showToast("Tidak Dapat Diajukan", "Anda sudah melakukan presensi HADIR hari ini. Status khusus tidak dapat diajukan.", "error");
            return;
        }
    }
    ...
    try {
        ...
        if (j.status === 'success' || j.result === 'success') {
            ...
            if (selectedStatus === 'HADIR') {
                ...
                if (timeVal < 1000) {
                    btnHadir.classList.add('btn-done');
                    btnHadir.innerHTML = '<i data-lucide="check-circle" size="28"></i><span>SUDAH QR HADIR</span>';
                } else {
                    btnPulang.classList.add('btn-done');
                    btnPulang.innerHTML = '<i data-lucide="check-circle" size="28"></i><span>SUDAH QR PULANG</span>';
                }
            }
            lucide.createIcons();
