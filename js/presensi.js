// ============================================================
// PRESENSI.JS - v2.8.0 (FIXED - SEPERTI RAPORT.JS)
// ============================================================
// PERBAIKAN:
// 1. Menggunakan safeFetchJSON (seperti raport.js) - FIXED
// 2. Fallback ke data kosong jika error - FIXED
// 3. Tidak ada CORS block yang ketat - FIXED
// 4. Mode Offline tanpa error merah - FIXED
// 5. Support Chrome, Opera, Firefox, Edge - FIXED
// ============================================================
...
// ============================================================
// 1. FETCH DENGAN TIMEOUT (SEPERTI RAPORT.JS)
// ============================================================
...
const STATUS_CONFIG = {
    ...
    'IZIN': {
        ...
        title: '📝 IZIN',
        message: 'Silahkan hubungi Koordinator / Pimpinan untuk meminta izin.',
        icon: 'file-text',
        color: '#d8b4fe',
        borderColor: '#a855f7',
    ...
    'SAKIT': {
        ...
        title: '🏥 SAKIT',
        message: 'Lampirkan foto surat keterangan dokter.',
        icon: 'heart-pulse',
        color: '#fde047',
    ...
    'DINAS': {
        ...
        title: '💼 DINAS',
        message: 'Lampirkan foto surat tugas dinas.',
        icon: 'briefcase',
        color: '#fdba74',
    ...
    'QUICK RESPONSE': {
        ...
        message: 'Tuliskan ringkasan tugas hari ini dengan detail.',
        icon: 'zap',
        color: '#f9a8d4',
        borderColor: '#ec4899',
        actions: []
...
function showToast(msg, type = 'info') {
    ...
    if (!c) { 
        ...
        c.id = 'toastContainer'; 
        c.style.cssText = 'position:fixed;top:20px;right:20px;z-index:100000;display:flex;flex-direction:column;gap:10px;pointer-events:none;max-width:380px;'; 
    ...
    t.innerHTML = `<div style="display:flex;align-items:center;gap:10px;"><span>${emoji[type] || 'ℹ️'}</span><div>${msg}</div></div>`;
    c.appendChild(t);
    setTimeout(() => { 
        t.style.opacity = '0'; 
...
function checkAtt(id, st) {
    ...
    const result = pegawaiRecords.some(l => {
        ...
        if (statusLower === 'hadir') {
            return ls.includes('hadir') || ls.includes('terlambat') || 
                   ls.includes('qr hadir') || ls.includes('qr terlambat');
        }
...
function loadAutoRecovery() {
    ...
    if (saved) {
        ...
        } catch(e) { sessionStorage.removeItem('pusda_recovery'); }
    }
}
...
async function ensureFaceApiLoaded() {
    ...
    try {
        ...
        isFaceApiLoading = false;
        showToast('Mode AI Gagal, menggunakan kamera standar', 'warning');
        return false;
    }
}
    ...
    navigator.geolocation.getCurrentPosition(
        (p) => {
            if (p.coords.accuracy > 250) {
                sndError.play();
                showToastOnce('gps_lemah', 'Sinyal Lemah', `Akurasi GPS ${p.coords.accuracy.toFixed(0)}m (>250m). Presensi ditolak.`, 'error');
            ...
            if (p.coords.accuracy > 150) {
                showToastOnce('gps_warning', 'Peringatan Sinyal', `Akurasi GPS ${p.coords.accuracy.toFixed(0)}m. Disarankan cari lokasi lebih terbuka.`, 'warning');
            }
...
// ============================================================
// 14. DATA FETCHING (SEPERTI RAPORT.JS)
// ============================================================
function loadFromCache() {
    ...
    if (c) {
        try {
            ...
            upUI();
            return true;
        } catch(e) { localStorage.removeItem('pusda_pegawai_v1'); return false; }
    }
    return false;
}
...
async function loadData() {
    ...
    try {
        ...
        const [d1, d2] = await Promise.all([
            safeFetchJSON(url, { redirect: 'follow' }, 15000),
            safeFetchJSON(urlPresensi, { redirect: 'follow' }, 15000)
        ]);
        ...
        if (fallbackCache) {
            ...
            if (statusText) statusText.innerText = "Mode Offline - Data Cache";
            ...
            showToast('Mode Offline - Menggunakan data cache', 'warning');
        } else {
            // ✅ TETAP TAMPILKAN UI meskipun tanpa data (seperti raport.js)
            if (statusText) statusText.innerText = "Mode Offline - Tanpa Data";
            document.getElementById('pName').innerText = "MODE OFFLINE";
            document.getElementById('pName').style.color = "var(--warning)";
            document.getElementById('pJob').innerText = "Tidak ada data tersimpan";
            document.getElementById('pImg').src = placeholderImg;
            showToast('Koneksi Gagal - Data mungkin tidak lengkap', 'warning');
        }
...
async function silentBackgroundUpdate() {
    try {
        const [d1, d2] = await Promise.all([
            safeFetchJSON(API + "?action=getDashboardData&cb=" + Date.now(), { redirect: 'follow' }, 15000),
            safeFetchJSON(API + "?action=getTodayPresensi&cb=" + Date.now(), { redirect: 'follow' }, 15000)
        ]);
        dbE = d1.pegawai || [];
        dbF = [...dbE];
        dbP = d2.data || [];
...
async function refreshPresensiData() {
    try {
        ...
        const data = await safeFetchJSON(API + "?action=getTodayPresensi&cb=" + Date.now(), {
            cache: 'no-cache'
        }, 15000);
...
function setS(el, st) {
    if (uPos.lat === 0 || !uPos.lat) {
        ...
        upLoc();
        setTimeout(() => { if (uPos.lat === 0) showToast('GPS Gagal, tunggu GPS terkunci.', 'error'); }, 3000);
        return;
    }
    ...
    if (outside && !exc) {
        sndError.play();
        showToast(`Ditolak - Anda berada di luar area (${g.jarak}m dari ${g.nama})`, 'error');
        const info = document.getElementById('statusInfo');
        info.style.display = 'block';
    ...
    if (st === 'HADIR') {
        if (checkAtt(pid, 'HADIR')) { sndError.play(); showToast('Sudah Absen - Anda sudah HADIR hari ini.', 'error'); return; }
    } else if (st === 'PULANG') {
        if (checkAtt(pid, 'PULANG')) { sndError.play(); showToast('Sudah Absen - Anda sudah PULANG hari ini.', 'error'); return; }
        if (!checkAtt(pid, 'HADIR')) { showToast('Urutan Salah - Harus HADIR dulu sebelum PULANG.', 'error'); return; }
    } else if (st === 'QUICK RESPONSE') {
        const isMorning = timeVal < jamPulangLimit;
        if (isMorning) {
            if (checkAtt(pid, 'HADIR')) { sndError.play(); showToast('Sudah Absen - Anda sudah HADIR/QR HADIR hari ini.', 'error'); return; }
        } else {
            if (checkAtt(pid, 'PULANG')) { sndError.play(); showToast('Sudah Absen - Anda sudah PULANG/QR PULANG hari ini.', 'error'); return; }
            if (!checkAtt(pid, 'HADIR')) { sndError.play(); showToast('Belum Absen Masuk - Harus HADIR/QR HADIR dulu.', 'error'); return; }
        }
    }
    document.getElementById('notes').value = '';
    updateNotesCounter();
...
async function submitWithRetry(attempt = 1, trxId = null) {
    ...
    if (uPos.lat === 0 || !uPos.lat) {
        ...
        await new Promise((resolve) => { upLoc(); setTimeout(resolve, 3000); });
        if (uPos.lat === 0) { return showToast('GPS Gagal - Tidak dapat mengambil lokasi.', 'error'); }
    }
    ...
    const statusMapping = {
        'HADIR': 'hadir', 'PULANG': 'pulang', 'IZIN': 'izin',
        'SAKIT': 'sakit', 'DINAS': 'dinas', 'QUICK RESPONSE': 'quick response'
    };
    ...
    const payload = {
        action: 'presensi',
        idPegawai: p.ID, nama: p.Nama, status: payloadStatus,
        selfie: sB64, workPhoto: kB64, surat: suratB64 || '-',
        keterangan: n, gps: `${uPos.lat},${uPos.lng}`,
        wilayah: p.Wilayah || "-", trxId: trxId
    };

    try {
        const j = await safeFetchJSON(API, {
            method: 'POST',
        ...
        if (j.status === 'success') {
            ...
            btnHadir.style.backgroundColor = ''; btnHadir.style.color = ''; btnHadir.style.borderColor = ''; btnHadir.style.boxShadow = '';
            btnPulang.style.backgroundColor = ''; btnPulang.style.color = ''; btnPulang.style.borderColor = ''; btnPulang.style.boxShadow = '';
            sessionStorage.removeItem('pusda_recovery');
            ...
            setTimeout(() => {
                ...
                if (peg) {
                    const params = new URLSearchParams({
                        id: peg.ID || peg.id, nama: peg.Nama || peg.nama,
                        jabatan: peg.Jabatan || 'PPA', wilayah: peg.Wilayah || 'UPT',
                        foto: peg.Link_Foto_Profile || '',
                        status: 'success', msg: `Presensi ${j.statusFix} berhasil! Nilai: ${j.nilai} pts`
                    });
...
async function openForm() {
    ...
    const targetId = activePegawai.ID || activePegawai.id;
    ...
    const rawUrl = activePegawai.Link_Foto_Profile || activePegawai.link_foto_profile || "";
    let finalSrc = placeholderImg;
    if (rawUrl) {
        if (rawUrl.includes('drive.google.com') || rawUrl.includes('googleusercontent.com')) {
            let fileId = "";
            let match = rawUrl.match(/\/d\/([^\/\?]+)/);
            if (match && match[1]) fileId = match[1];
            if (!fileId) { match = rawUrl.match(/[?&]id=([^&]+)/); if (match && match[1]) fileId = match[1]; }
            if (fileId) { finalSrc = `https://drive.google.com/thumbnail?id=${fileId}&sz=w500`; } 
            else { finalSrc = rawUrl; }
        } else { finalSrc = rawUrl; }
    }
    document.getElementById('formHeroImg').src = finalSrc;
    document.getElementById('formName').innerText = activePegawai.Nama || activePegawai.nama;
    document.getElementById('formJobWil').innerHTML =
        `<i data-lucide="briefcase" size="14" style="vertical-align:middle"></i> ${activePegawai.Jabatan || "PPA"} | <i data-lucide="map-pin" size="14" style="vertical-align:middle"></i> ${activePegawai.Wilayah || "UPT"}`;
    lucide.createIcons();
    ...
    btnHadir.style.pointerEvents = ''; btnHadir.style.opacity = ''; btnHadir.style.backgroundColor = ''; btnHadir.style.color = ''; btnHadir.style.borderColor = ''; btnHadir.style.boxShadow = '';

    btnPulang.classList.remove('btn-done', 'active');
    btnPulang.innerHTML = '<i data-lucide="moon" size="28"></i><span>PULANG</span>';
    btnPulang.style.pointerEvents = ''; btnPulang.style.opacity = ''; btnPulang.style.backgroundColor = ''; btnPulang.style.color = ''; btnPulang.style.borderColor = ''; btnPulang.style.boxShadow = '';

    const pid = activePegawai.ID || activePegawai.id;
    ...
    updateNotesCounter();
    updateWorkflow();
    setTimeout(() => { initMap(); upLoc(); loadAutoRecovery(); updateButtonColors(); }, 300);
    isFormLoading = false;
}
...
// ============================================================
// 20. PROFILE RAPORT NAVIGATION & REFRESH
// ============================================================
function goToProfileRaport() {
    const p = activePegawai || dbF[uIdx];
    if (!p) { showToast('Peringatan - Pilih pegawai terlebih dahulu.', 'warning'); return; }
    const params = new URLSearchParams({
        id: p.ID || p.id, nama: p.Nama || p.nama,
        jabatan: p.Jabatan || 'PPA', wilayah: p.Wilayah || 'UPT',
...
async function manualRefreshStatus() {
    ...
    if (btn) { btn.innerHTML = '<i data-lucide="refresh-cw" size="18" style="animation:spin 0.8s linear infinite"></i>'; lucide.createIcons(); }
    showToast('Memperbarui - Mengambil data presensi terbaru...', 'info');
    try {
        const success = await refreshPresensiData();
        if (success) { updateUIAfterRefresh(); showToast('Berhasil - Status presensi diperbarui.', 'success'); } 
        else { showToast('Peringatan - Data presensi kosong.', 'warning'); }
    } catch(e) { console.error('❌ Refresh error:', e); showToast('Gagal - Periksa koneksi.', 'error'); }
    if (btn) { btn.innerHTML = '<i data-lucide="refresh-cw" size="18"></i>'; lucide.createIcons(); }
}

async function forceUpdateStatus() {
    console.log('🔄 Force update status...');
    const success = await refreshPresensiData();
    if (success) { updateUIAfterRefresh(); showToast('Status Diperbarui - Berhasil.', 'success'); console.log('✅ Status updated successfully'); } 
    else { showToast('Gagal - Coba lagi.', 'error'); console.log('❌ Status update failed'); }
}
...
function updateUIAfterRefresh() {
    ...
    if (!isFormOpen) { console.log('⚠️ Form tidak terbuka, skip update UI'); return; }
    const p = activePegawai || dbF[uIdx];
    if (!p) { console.log('⚠️ Tidak ada pegawai aktif'); return; }
    const pid = p.ID || p.id;
    const btnHadir = document.getElementById('btnHadirMain');
    const btnPulang = document.getElementById('btnPulangMain');
    if (!btnHadir || !btnPulang) { console.log('⚠️ Tombol tidak ditemukan'); return; }
    ...
    btnHadir.innerHTML = '<i data-lucide="sun" size="28"></i><span>HADIR</span>';
    btnHadir.style.pointerEvents = ''; btnHadir.style.backgroundColor = ''; btnHadir.style.color = ''; btnHadir.style.borderColor = ''; btnHadir.style.boxShadow = '';
    btnPulang.innerHTML = '<i data-lucide="moon" size="28"></i><span>PULANG</span>';
    btnPulang.style.pointerEvents = ''; btnPulang.style.backgroundColor = ''; btnPulang.style.color = ''; btnPulang.style.borderColor = ''; btnPulang.style.boxShadow = '';
    if (hadirStatus) { btnHadir.classList.add('btn-done'); btnHadir.innerHTML = '<i data-lucide="check-circle" size="28"></i><span>SUDAH HADIR</span>'; btnHadir.style.pointerEvents = 'none'; }
    if (pulangStatus) { btnPulang.classList.add('btn-done'); btnPulang.innerHTML = '<i data-lucide="check-circle" size="28"></i><span>SUDAH PULANG</span>'; btnPulang.style.pointerEvents = 'none'; }
    updateAttendanceStatusIndicator();
    lucide.createIcons();
}
...
function detectReturnFromProfile() {
    ...
    if (justReturned === 'true') {
        ...
        refreshPresensiData().then((success) => {
            if (success) { updateUIAfterRefresh(); showToast('Data Diperbarui - Status presensi telah diperbarui.', 'success'); }
        }).catch((err) => { console.warn('⚠️ Gagal refresh data:', err); });
    }
}

// ============================================================
// 21. CANVAS SETUP (UNTUK CAMERA OVERLAY)
// ============================================================
...
// ============================================================
// 22. CAMERA FUNCTIONS (LENGKAP - SEPERTI SEBELUMNYA)
// ============================================================
async function triggerCam(type) {
    ...
    if (!selectedStatus) { showToast('Peringatan - Pilih status presensi dulu!', 'warning'); return; }
    if (notes.length < 5) { showToast('Peringatan - Isi keterangan minimal 5 karakter!', 'warning'); return; }
    cType = type;
    stopCurrentStream();
    ...
    try {
        ...
        video.onloadedmetadata = () => {
            ...
            }).catch(e => { showToast('Error Kamera - Gagal memutar video.', 'error'); stopCam(); });
        };
    } catch(err) {
...
function triggerGallery() {
    if (!selectedStatus) { showToast('Peringatan - Pilih status presensi dulu!', 'warning'); return; }
    if (document.getElementById('notes').value.trim().length < 5) { showToast('Peringatan - Isi keterangan minimal 5 karakter!', 'warning'); return; }
    ...
    img.onload = () => {
        ...
        if (type === 'selfie' && isFaceApiLoaded) {
            faceapi.detectSingleFace(c, new faceapi.TinyFaceDetectorOptions({ inputSize: 320, scoreThreshold: .3 })).then(d => {
                if (!d) { sndError.play(); showToast('Gagal Deteksi - Wajah tidak terdeteksi!', 'error'); return; }
                addWatermark(c); savePhoto(c, type);
...
function savePhoto(c, type) {
    ...
    sndShutter.play();
    if (type === 'selfie') {
        document.getElementById('sImg').src = d; document.getElementById('sImg').style.display = 'block';
        document.getElementById('sPh').style.display = 'none'; sB64 = d;
    } else {
        document.getElementById('kImg').src = d; document.getElementById('kImg').style.display = 'block';
        document.getElementById('kPh').style.display = 'none'; kB64 = d;
    }
...
function addWatermark(c) {
    ...
    const nameFontSize = Math.round(baseSize * 0.032), jobFontSize = Math.round(baseSize * 0.022);
    const infoFontSize = Math.round(baseSize * 0.020), footerFontSize = Math.round(baseSize * 0.018);
    const iconSize = Math.round(baseSize * 0.025), logoSize = Math.round(baseSize * 0.09);
    ...
    let displayName = nama, metrics = ctx.measureText(displayName + ' • ' + jabatan);
    ...
    ctx.restore();
    const line2Y = logoY + logoSize * 0.58, iconColor = '#2dd4bf', textColor = '#ffffff';
    ctx.save();
    ctx.textBaseline = 'middle';
    ctx.shadowColor = shadowConfig.shadowColor;
    ctx.shadowBlur = shadowConfig.shadowBlur;
...
function drawMapPinIcon(ctx, x, y, size, color) {
    ...
    ctx.lineWidth = size * 0.1; ctx.lineCap = 'round'; ctx.lineJoin = 'round';
...
function drawCalendarIcon(ctx, x, y, size, color) {
    ...
    ctx.beginPath(); ctx.moveTo(rx + w * 0.28, ry - pad * 0.5); ctx.lineTo(rx + w * 0.28, ry + pad * 0.5);
    ctx.moveTo(rx + w * 0.72, ry - pad * 0.5); ctx.lineTo(rx + w * 0.72, ry + pad * 0.5); ctx.stroke();
    ctx.restore();
}

// ============================================================
// 25. CAMERA OVERLAY FUNCTIONS (SINGKAT)
// ============================================================
...
// ============================================================
// 26. CANVAS DRAWING HELPERS (SINGKAT)
// ============================================================
...
function drawFaceGuide(ctx, W, H, color, glowColor) {
    const cx = W / 2, cy = H * 0.40, rx = Math.min(W, H) * .25, ry = Math.min(W, H) * .32, t = performance.now() / 1000;
    ctx.save();
    ctx.setLineDash([15, 10]); ctx.lineDashOffset = -t * 40;
    ...
    ctx.beginPath(); ctx.moveTo(cx - cs, cy); ctx.lineTo(cx - 5, cy); ctx.moveTo(cx + 5, cy); ctx.lineTo(cx + cs, cy);
...
function drawFaceWireframe(ctx, detection, W, H, color) {
    ...
    ctx.save();
    ctx.strokeStyle = color.replace('0.9', '0.4'); ctx.lineWidth = 1; ctx.setLineDash([5, 5]);
...
function drawRuleOfThirds(ctx, W, H) {
    ctx.save();
    ctx.strokeStyle = 'rgba(34,211,238,0.25)'; ctx.lineWidth = 1; ctx.setLineDash([6, 6]);
...
function drawCrosshair(ctx, W, H, color) {
    ...
    if (DeviceProfile.config.enableShadowBlur) { ctx.shadowColor = color; ctx.shadowBlur = 8; }
    ctx.beginPath(); ctx.moveTo(cx - outer, cy); ctx.lineTo(cx - gap, cy); ctx.moveTo(cx + gap, cy); ctx.lineTo(cx + outer, cy);
    ctx.moveTo(cx, cy - outer); ctx.lineTo(cx, cy - gap); ctx.moveTo(cx, cy + gap); ctx.lineTo(cx, cy + outer); ctx.stroke();
    ctx.shadowBlur = 0; ctx.fillStyle = color; ctx.beginPath(); ctx.arc(cx, cy, 2, 0, Math.PI * 2); ctx.fill();
    ctx.restore();
}
...
function drawWorkLabel(ctx, W, H) {
    ...
    const textW = ctx.measureText(label).width, padX = 12, padY = 6, x = 20, y = H * 0.12;
...
function showUpdateModal() {
    ...
    document.getElementById('notifMessage').innerText = "Versi terbaru aplikasi E-PUSDA telah dirilis. Muat ulang halaman untuk mendapatkan fitur terbaru.";
...
window.onload = () => {
    ...
    document.addEventListener('visibilitychange', () => {
        if (!document.hidden) {
            ...
            refreshPresensiData().then(() => { updateUIAfterRefresh(); updateButtonColors(); });
        }
    });
    ...
    } catch(e) { console.warn('⚠️ Service Worker error:', e.message); }
};
