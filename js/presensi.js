...
// ============================================================
// 9B. CHECK TODAY STATUS (FIXED - TIMEZONE PROOF)
// ============================================================
function checkTodayStatus(pid) {
    // ✅ FIX 1: dbP dari getTodayPresensi SUDAH difilter oleh backend untuk hari ini (WIB).
    // Jadi kita TIDAK PERLU filter tanggal lagi di frontend. 
    // Ini menghindari bug timezone UTC vs WIB yang menyebabkan record tidak terdeteksi.
    
    const targetId = String(pid).trim().toLowerCase();
    
    // ✅ FIX 2: Filter hanya berdasarkan ID, abaikan filter tanggal yang rawan bug
    const todayRecords = dbP.filter(r => {
        ...
        // Handle kemungkinan ID angka vs string (misal "1234" vs "01234")
        const cleanRecordId = recordId.replace(/^0+/, '');
        const cleanTargetId = targetId.replace(/^0+/, '');
        
        return cleanRecordId === cleanTargetId || recordId === targetId;
    });

    console.log(`📊 checkTodayStatus(${pid}):`);
    console.log(`   Target ID: ${targetId}`);
    console.log(`   Total records in dbP: ${dbP.length}`);
    console.log(`   Records matched for this pegawai: ${todayRecords.length}`);
